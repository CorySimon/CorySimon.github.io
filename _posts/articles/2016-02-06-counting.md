...
##### Case 1. Christina, Joaquin, and Ezra each select one fruit. There are plenty of bananas :banana:, strawberries :strawberry:, watermelons :watermelon:, and green apples :green_apples: in the cafeteria. How many different outcomes are there? e.g., one outcome is Christina and Ezra select a banana and Joaquin selects a watermelon.
...
In this particular case, $$\binom{2+4-1}{2}=10$$. We now check by enumerating the possibilities. We have four more possibilities than in Case 5 above, since we now allow for dupilicates:
