...
During the Monte Carlo simulations of the grand canonical ensemble (GCMC simulations), we compute the energy of the adsorbate molecules, $$E$$, at each sample. One might postulate that the isosteric heat of adsorption is related to the average value of $$E/N$$ during the simulation, where $$N$$ is the number of adsorbates in the system. This is incorrect because it biases the estimate of the energy per adsorbed molecule towards when there are very few particles in the system.

Instead, we define the average energy of adsorption $$q$$ as the difference in average energy of the system upon the addition of a single adsorbate molecule:
...
To get a more useful formula for estimating $$q$$ from GCMC simulations, we need to bring in the grand canonical partition function $$\Xi(\beta, \mu, V)$$, where $$\beta=1/T$$, $$\mu$$ is the chemical potential of the gas, and $$V$$ is the volume of the adsorbates. Although a bit abstract, we can write the partition function as a sum over particles in the system $$N$$ and microstates $$\nu$$ with that particle number:

$$\Xi = \displaystyle \sum_{N=0}^{\infty} \displaystyle \sum_{\nu} e^{-\beta E_{\nu} + \beta \mu N}$$

Here, $$E_{\nu}$$ characterizes the energy of that particular microstate $$\nu$$.

The probability of a microstate $$\mu$$ with $$N$$ particles is then $$1 / \Xi e^{-\beta E_{\mu} + \beta \mu N}$$. From this and the expectation value formula from elementary probability theory, we can work out the relationship between the partition function and the average energy and particle number:
...
$$q \approx \frac{\partial \langle E\rangle}{\partial \langle N \rangle} = \frac{\partial \langle E\rangle}{(\beta \mu)} \frac{\partial (\beta \mu)}{\partial \langle N \rangle}.$$
...
$$\frac{\partial \langle N \rangle}{(\beta \mu)}= \langle N ^2 \rangle - \langle N \rangle ^2 = var(N)$$

As for the first derivative, we use the relationship we derived:

$$\frac{\partial \langle E\rangle}{(\beta \mu)} = -\frac{\partial \langle N\rangle}{\partial \beta} = -\frac{\partial}{\partial \beta} \displaystyle \sum_{N=0}^{\infty} \displaystyle \sum_{\nu} N \frac{e^{-\beta E_{\nu} + \beta \mu N}}{\Xi}.$$

Here, we wrote out the expectation of $$N$$ from elementary probability theory using $$p(N) = \displaystyle \sum_{\nu} \frac{e^{-\beta E_{\nu} + \beta \mu N}}{\Xi}$$.
...
You may be wondering why we did not consider the $$e^{\beta \mu N}$$ term during differentiation, but actually we are viewing $$\Xi$$ as $$\Xi = \Xi(\beta, \beta \mu, V)$$ here.
...
$$\frac{\partial \langle E\rangle}{(\beta \mu)} = \langle E N \rangle - \langle E \rangle \langle N \rangle.$$

Finally, we arrive at the average energy of adsoption as:

$$q =  \frac{\langle E N \rangle - \langle E \rangle \langle N \rangle}{var(N)},$$

which are all statistical quantities we can easily measure during our Monte Carlo simulation samples. This formula is fascinating because it relates the average energy of adsorption to fluctuations in the particle number and the covariance of the energy and particle number; in a sense, this is another fluctuation theorem in statistical mechanics.
